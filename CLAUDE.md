...
2. [ ] FastAPI + 카카오 소셜 로그인 + DB(SQLite)로 멀티유저 구조 전환
   ...
   - 2-2. 관심 키워드 "직접 입력" 플로우 (그룹 생성 + 키워드 저장 API + 마이페이지 조회)
...
2-1(카카오 소셜 로그인) 완료됨. 다음은 2-2(관심 키워드 "직접 입력" 플로우:
그룹 생성 + 키워드 저장 API + 마이페이지 조회)부터 진행. 진행하기 전에 이
파일의 로드맵 체크리스트를 업데이트해서 어디까지 했는지 계속 반영할 것.